...
describe('POST /vehicles/:id (bumpup)', function () {
  ...
  it('bump up by an unauthorized user', function (done) {
    sera.model('vehicles').update({_id: vehicle.id}, {updatedAt: Date.now() - BUMP_UP_THRESHOLD}, function (err) {
      if (err) {
        return done(err);
      }
      request({
  ...
  describe('bump up on published vehicle', function () {
    ...
    it('bump up by an authorized user (too early)', function (done) {
      sera.model('vehicles').update({_id: vehicle.id}, {updatedAt: Date.now()}, function (err) {
        if (err) {
          return done(err);
        }
        request({
